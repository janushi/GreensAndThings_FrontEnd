...
$(".update").click(function(e) {                
        //alert("hi");
          ...
          var update_cid = $("#categoryname").val();

          var update_name=$("#productname").val();
         // alert(update_id);
          //alert(update_name);
          var qrystrng = 'id='+update_id+'&'+'name='+update_name+'&'+'id1='+update_cid;
          alert(qrystrng);
           $.ajax({    //create an ajax request to load_page.php
            type: "POST",
            url: "checkupdateproductnx.php",             
            data:qrystrng,
          ...
          else if(($.trim(response))=='Product already exists.')
          {
...
<div class="page-body">
<form role="form" method="post" action="#" data-parsley-validate id="addproductform">
...
<?php
 $qry122 = "SELECT * FROM product WHERE pid=".$id."";
        // echo $qry;
...
<?php
$qry66 = "SELECT * FROM category WHERE cid!=".$id1."";
        // echo $qry;
        $rs66 = mysqli_query($conn,$qry66);

                                    if (mysqli_num_rows($rs66) > 0) {
                                        // output data of each row
                                        while($row66 = mysqli_fetch_assoc($rs66)) 
                                        {
                                 ?>


<option value="<?php echo $row66['cid'];?>"><?php echo $row66['name']; ?></option>
<?php } } ?>

<?php
$qry = "SELECT * FROM category WHERE cid=".$id1."";
        // echo $qry;
 ...
 $qry1 = "SELECT * FROM product WHERE pid=".$id."";
        ...
        $rs1 = mysqli_query($conn,$qry1);
                                    ...
                                    if (mysqli_num_rows($rs1) > 0) {
                                        ...
                                        {
                                              ?>
<input id="productname" name="productname" type="text" placeholder="Add Product" value="<?php echo $row1['name'];?>" class="form-control" aria-label="Text input with dropdown button">
  <?php }} ?>
</div>
</div>
</div>
...
<div class="col-sm-12">
<button style="width:50%" id="<?php echo $id ?>" id1="<?php echo $id1 ?>" name="submit" type="submit" class="btn btn-secondary update">
Add
</button>
</div>
</div>
